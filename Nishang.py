				...
				try:
					...
					if "logged_in_user" in str(xyaaXD) or "sessionid" in ses.cookies.get_dict():
						...
						try:
							...
							tree.add(f"{P2}NAMA : {H2}{nama}").add(f"{H2}{user}|{pw}")
							tree.add(f"{P2}PENGIKUT : {H2}{pengikut}")
							tree.add(f"{P2}MENGIKUTI : {H2}{mengikut}")
							tree.add(f"{P2}NO HAPE  : {H2}{nomor}")
						...
						nama,pengikut,mengikut,postingan=self.APIinfo(user)
						tree = Tree("")
						tree.add(Panel(f"{K2}   Ｌｏｇｉｎ Ｃｈｅｃｋｐｏｉｎｔ {P2}",width=30,padding=(0,2),style=f"green"))
						tree.add(f"{P2}NAMA : {K2}{nama}")
						tree.add(f"{P2}USERNAMA  : {K2}{user}")
						tree.add(f"{P2}KATASANDI  : {K2}{pw}")
						tree.add(f"{P2}PENGIKUT : {K2}{pengikut}")
						tree.add(f"{P2}MENGIKUTI : {K2}{mengikut}")
